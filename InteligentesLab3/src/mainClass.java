	...
	private static int nextPosAvailable(int position, int [] distribution, Field field) {
		for(int i = position+1; i<distribution.length; i++) {
			if(positions[i-1]) { //Falta comprobar que no sea mayor que el máximo
				if(i-1 == 0 && field.getNumber(field.getXt()-1, field.getYt())+distribution[1]<field.getMax()) //Al norte
					return i;
				if(i-1 == 1 && field.getNumber(field.getXt(), field.getYt()-1)+distribution[2]<field.getMax()) //Al oeste
					return i;
				if(i-1 == 2 && field.getNumber(field.getXt(), field.getYt()+1)+distribution[3]<field.getMax()) //Al este
					return i;
				if(i-1 == 3 && field.getNumber(field.getXt()+1, field.getYt())+distribution[4]<field.getMax()) //Al sur
					return i;
			}
		}
